...
Iris's exact `CreatePayout`/`ApprovePayout` JSON request/response field names could not be confirmed through available documentation tools in this session — only the method names, separate-API-key auth, and two-step creator→approver workflow were confirmed (via the official `midtrans-go` client's method signatures). **The actual HTTP call bodies inside `MidtransIrisGateway` are a flagged pre-implementation TODO**, not invented field names — every other piece of this spec (interface, feature flag, bank dropdown, processing status, polling job, tests via a fake) is real, buildable, testable code. Confirm the exact payload shape against `https://docs.midtrans.com/reference/` (or a Postman collection, if available) before implementing that one piece.
